...
if sect == "Homepage":
	...
	st.markdown(f'<h3 style="color:\'red\';font-size:20px;">Please enter the following information:</h3>', unsafe_allow_html=True)
	...
	if "@" not in email:
		st.markdown(f'<h4 style="color:\'red\';font-size:18px;">Invalid email address</h4>', unsafe_allow_html=True)
	else:
	...
	try:
		...
		st.markdown(f'<h4 style="color:\'red\';font-size:18px;">Invalid card number</h4>', unsafe_allow_html=True)
